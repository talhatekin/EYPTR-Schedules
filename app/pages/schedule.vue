<template>
  <div class="space-y-6">
    <!-- Başlık ve Saat -->
    <div
      class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-4"
    >
      <div>
        <h1
        ...
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">
          Events, workshops, and timings by day.
        </p>
      </div>

      <!-- Gerçek Zamanlı Saat -->
      <div
        class="flex items-center gap-2 bg-slate-100 dark:bg-white/[0.06] border border-slate-200 dark:border-white/10 px-3.5 py-2 rounded-xl self-start sm:self-auto"
      >
        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
        <span
          class="text-sm font-semibold text-slate-700 dark:text-slate-200 tabular-nums"
          >Istanbul {{ currentTime }}</span
        >
      </div>
    </div>
    ...
    <!-- ROL SEÇİM ANAHTARI -->
    <div
      class="bg-slate-100 dark:bg-white/[0.06] p-1.5 rounded-xl border border-slate-200 dark:border-white/10 flex items-center gap-1.5 w-full sm:w-auto sm:inline-flex"
    >
      <button
        v-for="role in roles"
        :key="role.value"
      ...
      >
        ...
        <span
          :class="[
            ...
            selectedDayId === day.id
              ? 'bg-white/15 text-white dark:bg-black/25'
    ...
    <div
      v-if="selectedDayObj"
      class="bg-slate-900 dark:bg-white/[0.06] p-4 sm:p-5 rounded-2xl border border-slate-900 dark:border-white/10 flex items-center justify-between gap-3 text-white"
    >
      <div class="flex items-center gap-3.5 min-w-0">
        <div
          class="w-11 h-11 rounded-xl bg-white/10 dark:bg-white/10 flex items-center justify-center shrink-0"
        >
          <AppIcon
            :name="selectedRole === 'delegates' ? 'user' : 'briefcase'"
            :size="22"
          />
        </div>
        <div class="min-w-0">
          <h2 class="font-semibold text-base sm:text-lg leading-tight truncate">
            {{ selectedDayObj.label }}
          </h2>
          <p class="text-sm text-slate-300 dark:text-slate-400">
            Timings for
            {{ selectedRole === "delegates" ? "trainee" : "officials" }}
          </p>
        </div>
      </div>
      <div
        class="flex items-center gap-1.5 bg-white/10 text-slate-100 font-semibold text-sm px-3.5 py-2 rounded-xl shrink-0"
      >
    ...
    <div
      v-if="
        sortedDays && sortedDays.length > 0 && currentDaySessions.length > 0
      "
      class="space-y-3"
    >
      <div
      ...
      >
        <div class="flex items-start gap-3.5 min-w-0">
          ...
          <div class="space-y-1 min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              <span
                v-if="isSessionLive(session)"
                class="inline-flex items-center gap-1 bg-rose-500 text-white text-[11px] font-semibold px-2 py-0.5 rounded-md uppercase tracking-wide"
              >
                <span
              ...
              <h3
                class="font-semibold text-base sm:text-lg text-slate-900 dark:text-white leading-snug"
              >
                {{ session.title }}
              </h3>
            </div>
        ...
        <div
          ...
          class="flex items-center gap-1.5 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 px-3 py-1.5 rounded-xl self-start sm:self-center shrink-0 text-slate-700 dark:text-slate-200"
        >
          <AppIcon
            name="map-pin"
            :size="15"
    ...
    <div
      v-else
      class="text-center py-16 px-6 bg-white dark:bg-white/[0.03] rounded-2xl border border-dashed border-slate-300 dark:border-white/10 space-y-3"
    >
      <div
        class="w-12 h-12 mx-auto rounded-xl bg-slate-100 dark:bg-white/[0.06] flex items-center justify-center text-slate-400 dark:text-slate-500"
      >
        <AppIcon
          :name="selectedRole === 'delegates' ? 'user' : 'briefcase'"
          :size="24"
      ...
      <h3 class="font-semibold text-slate-800 dark:text-slate-200 text-base">
        No schedule published yet
      </h3>
      <p class="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto">
        There are no sessions listed for
        {{ selectedRole === "delegates" ? "trainee" : "officials" }} right
...
const roles = [
  { value: "delegates", label: "Trainee", icon: "user" },
...
const nowInConfTz = (ts) =>
  new Date(new Date(ts).toLocaleString("en-US", { timeZone: CONF_TZ }));
...
const isSessionLive = (session) => {
  ...
  const parts = String(session.time)
    .split(DASH_RE)
...
const currentDaySessions = computed(() => {
  ...
  return [...sessions.value]
    .filter(
      (s) =>
        s.dayId === selectedDayId.value &&
        (s.role || "delegates") === selectedRole.value,
    )
...
watch(
  ...
  (newVal) => {
    if (
      newVal.length > 0 &&
      (!selectedDayId.value ||
        !newVal.find((d) => d.id === selectedDayId.value))
    ) {
